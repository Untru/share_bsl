...
14. [Асинхронное выполнение](#асинхронное-выполнение)
...
Перед тем как погрузится с головой в наше приложение, давайте напишем что то простое.
Любое приложение на Осени начинается с точки входа. Создадим три файла:
...
```bsl
// Классы\ПервыйЗапущенныйКласс.os
...
Процедура Поздароваться() Экспорт
	Сообщить(ФразаПриветствия);
КонецПроцедуры
```
Запустим  `oscript main.os` И увидем вывод:
...
Что тут происходит?
В момент ЗапускПриложения() у нас происходит регистрация наших компонентов. 

Компоненты у нас называются желуди, но пусть тебя не отвлекает нейминг от сути.
В примере выше компонент - Класс `ПриветМир!` Что бы его объявить - у метода `ПриСозданииОбъекта()` мы установили аннотацию `&Желудь` — так мы указали, что этот класс мы можем получить в любой момент из Контейнера, создаваемого Осенью.

Когда мы пишем `&Пластилин`, мы получаем экземпляр нашего объекта.

Как можно более простым языком это описать? Если очень грубо, то давайте представим, что у нас есть глобальный контекст и есть глобальная переменная с типом `Соответствие`.
Ключ — имя класса, значение — сам объект. В любом месте мы можем получить его и использовать.
Когда мы на какой то класс вешаем аннотацию `&Рогатка` - этот класс создается в момент запуска.
Если разобрать по шагам, у нас получается:
1. Поделка.ЗапуститьПриложение();
2. Так как у `ПервыйЗапущенныйКласс` аннотация `&Рогатка` Он создается следующим.
3. У класса есть `&Пластилин` - ПриветМир , мы создаем его и помещаем в контекст приложения.
4. При создании `ПриветМир` у свойства `ФразаПриветствия` устанавливается значение `"Привет, мир!"`
по этому когда
```bsl
Процедура ПриЗапускеПриложения() Экспорт
...
5. Мы устанавливаем новое значение свойству `"ПриветМир.ФразаПриветствия = "Hello, world!";`
6. В строке `ПриветМир = Поделка.НайтиЖелудь("ПриветМир");` Мы достаем уже существующий компонент.
7. `ПриветМир.Поздароваться();` у нас уже вывод `"Hello, world!"`

Если бы не было Осени, нам бы пришлось продумывать как передавать объекты свойства классов. Но вернемся к нашей задаче!
...
Теперь разберем код. У нас есть класс, который реализует логику общения с Telegram, мы его назвали `ТелеграммБот`. Чтобы начать работать с этим классом, у нас есть два пути:
...
В одном месте мы заполняем токен, потом в любых других местах через аннотацию `&Пластилин` мы получаем экземпляр этого класса, а он уже настроен так, как нам нужно, так же, нам не надо думать о том как этот экземпляр передать в нужныем нам классы.
...
У метода `ПриСозданииОбъекта()` установили аннотацию `&Желудь` — напомню, так мы указали, что этот класс мы можем получить в любой момент из Контейнера, создаваемого Осенью. Именно так мы и сделали в классе Запускатор.
...
1. Хороший тон для любого опенсорс-проекта — это наличие тестов. Давайте придумаем тесты для нашего бота и обсудим их в комментариях к статье. 
2. У нас не все хорошо с ответственностью она растеклась по командам, подумай можем ли мы сделать отдельный желудь `ОтправляторCообщений` который будет знать токен и отправлять сообщения пользователь
3. Задача со звездочкой, сделать `ОтправляторCообщений` с прозвещем `Уведомдения` тогда например мы можем отвечать не только в Telegram но и в почту
