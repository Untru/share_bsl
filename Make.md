...
У нас с вами почти все готово, файлы отправляются, код присылается, чего еще желать? Давайте смоделируем такую ситуацию, мне отправляют обработку, я ее смотрю и оставляю свои коментарии, программист правит код, и отправляет обратно, было бы удобно заменять публикацию новой, мало того, возможности Gist нам покажут, какой код был изменен.
Но как это сделать, нам же надо хранить где то информацию и том, какие публикации были уже отправлены. Предлагаю подключить к нашему боту базу данных. И тут тоже Осень готова нам помочь. Нарисуем взаимодействие пользователя с телеграмм.
