...
Хороший программист, ленивый программист, давайте использовать готовую библиотеку, мне нравится библиотека Антона [ОПИ](https://openintegrations.dev/), поражает подход к написанию и развитию этой библиотеки. Внимание ко всем мелочам.
...
#КонецОбласти
```
И так разберем, что тут написано?
 
Мы подключили oint (ОПИ) что бы использовать уже готовые интеграции с телеграмм.

У метода ПриСозданииОбъекта() установили аннатацию &Желудь, так мы указали, что этот класс мы можем достать в любой момент из Контейнера создаваемого осенью, например как мы сделали в Запускатор.os. При инициализации приложения и обращении к классу  
А когда мы пишем &Пластилин - мы достаем наш объект.
&Число - можем указать тип данных, это тоже возможности осени

На этом этапе у нас с вами готова основа, у нас даже сейчас бот будет отвечать эхом, можно запустить отладку, дя удобства, прпопишем в отладку запускаемый файл, тогда не смотря на спозициолнирвоанный в проекте файл все равно запустится нужный.
...
Внимательный читатель наверняка заметил что очень не очень писать токен в коде, особенно когда потом проект идет в публичный репозитооий github, есть даже роботы которые сканируют ГХ на предмет разных секретов
мы могли бы создать конфигурационный файл с настройками, напрограмировать его чтение, но осень все сделала за нас.

Нам всего то нужно создать файлик autumn-properties.json и указать все секреты там. Не забывайте указывать его в .gitignore, а то секреты в итоге всеравно улетят в публичный доступ
...
```
Теперь что бы достать нашу "Настройку" - нам нужно воспользоватьсмя аннатацией &Деталька
![alt text](images/image-6.png)

## Добавляем логирование

Как и в первой части, нам пора позаботится о логировании нашего приложения, 





Читать проверять до этого места.



[Prototype vs Singleton ](https://dzen.ru/a/ZIOUQZSvqls4cP4g?ysclid=me7mesdvj690470205)
